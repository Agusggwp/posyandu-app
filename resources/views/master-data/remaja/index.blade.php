...
<div class="max-w-7xl mx-auto px-1 sm:px-0">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-8">
        ...
        <div class="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            <a href="{{ route('remaja.export-excel') }}" class="w-full sm:w-auto text-center bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2 px-4 rounded-xl shadow-md transition-all duration-200 flex items-center justify-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                Export Excel
            </a>
            <button onclick="toggleImportModal(true)" class="w-full sm:w-auto text-center bg-sky-700 hover:bg-sky-850 text-white font-semibold py-2 px-4 rounded-xl shadow-md transition-all duration-200 flex items-center justify-center gap-2">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm font-medium">
                            <div class="flex items-center gap-2">
                                ...
                                <form action="{{ route('remaja.destroy', $remaja->id) }}" method="POST" style="display: contents" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
...
<div id="importModal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form action="{{ route('remaja.import-excel') }}" method="POST" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-2 text-sm text-gray-500">
                                <p class="mb-4">Silakan unggah file Excel/CSV data remaja. Gunakan tombol di bawah ini untuk mengunduh template format data yang sesuai.</p>
                                <a href="{{ route('remaja.import-template') }}" class="inline-flex items-center gap-1.5 text-sky-700 hover:text-sky-900 font-semibold mb-6">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                                    Unduh Template Format CSV
                                </a>
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Pilih File (.csv)</label>
                                    <input type="file" name="file" accept=".csv" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="progress-container hidden bg-white px-4 pt-5 pb-8 sm:p-6 text-center">
                    <div class="flex justify-center mb-6">
                        <div class="relative w-16 h-16">
                            ...
                            <div class="absolute inset-0 rounded-full border-4 border-t-sky-700 border-r-sky-700 animate-spin"></div>
                        </div>
                    </div>
                    ...
                    <div class="w-full bg-gray-100 rounded-full h-3 relative overflow-hidden mb-2">
                        <div class="progress-bar-fill bg-gradient-to-r from-sky-500 to-sky-700 h-full rounded-full transition-all duration-300" style="width: 0%"></div>
                    </div>
                    <div class="text-sm font-bold text-sky-700 progress-percent">0%</div>
                </div>

                <div class="modal-footer bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                    <button type="submit" class="submit-btn w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 bg-sky-700 text-base font-semibold text-white hover:bg-sky-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 sm:ml-3 sm:w-auto sm:text-sm">
                        Mulai Import
                    </button>
                    <button type="button" onclick="toggleImportModal(false)" class="cancel-btn mt-3 w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                        Batal
                    </button>
                </div>
            </form>
