...
<div class="max-w-7xl mx-auto px-3 sm:px-4">
    ...
    <!-- Search Bar -->
    <form method="GET" action="{{ route('pemeriksaan-balita.index') }}" class="mb-6">
        <div class="flex gap-2 flex-col sm:flex-row">
            <div class="flex-1">
                <input type="text" name="search" placeholder="Cari nama balita..." value="{{ request('search') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
            </div>
            <button type="submit" class="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-semibold py-2 px-6 rounded-lg shadow-md hover:shadow-lg transition-all duration-200">Cari</button>
            @if(request('search'))
            <a href="{{ route('pemeriksaan-balita.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white font-semibold py-2 px-6 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 text-center">Hapus Filter</a>
            @endif
        </div>
    </form>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center gap-2">
                                ...
                                @endcan
                                @can('delete_data')
                                <form action="{{ route('pemeriksaan-balita.destroy', $pemeriksaan->id) }}" method="POST" style="display: contents"
        ...
        @forelse($pemeriksaans as $pemeriksaan)
            <div class="bg-white rounded-xl shadow p-4 border border-slate-100">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <h3 class="font-semibold text-slate-900">{{ $pemeriksaan->balita->nama_bayi ?? '-' }}</h3>
