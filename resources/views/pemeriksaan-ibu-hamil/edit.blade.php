        ...
        <form action="{{ route('pemeriksaan-ibu-hamil.update', $pemeriksaan->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="ibu_hamil_identitas_id" id="ibu_hamil_identitas_id" 
                        ...
                        @foreach($ibuHamils as $ibuHamil)
                            <option value="{{ $ibuHamil->id }}" {{ old('ibu_hamil_identitas_id', $pemeriksaan->ibu_hamil_identitas_id) == $ibuHamil->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_kunjungan" id="tanggal_kunjungan" 
                           value="{{ old('tanggal_kunjungan', optional($pemeriksaan->tanggal_kunjungan)->format('Y-m-d')) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    ...
                    <input type="text" name="kondisi_ibu" id="kondisi_ibu" 
                           value="{{ old('kondisi_ibu', $pemeriksaan->kondisi_ibu) }}"
                ...
                <div>
                    ...
                    <input type="text" name="petugas" id="petugas" 
                           value="{{ old('petugas', $pemeriksaan->petugas) }}"
            ...
            <div class="mt-6">
                ...
                <textarea name="keluhan" id="keluhan" rows="2"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('keluhan') border-red-500 @enderror">{{ old('keluhan', $pemeriksaan->keluhan) }}</textarea>
                @error('keluhan')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="catatan" id="catatan" rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('catatan') border-red-500 @enderror">{{ old('catatan', $pemeriksaan->catatan) }}</textarea>
                @error('catatan')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
