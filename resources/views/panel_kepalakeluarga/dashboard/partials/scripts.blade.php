<script>
    ...
    function getCheckupDetailText(checkup) {
        if (checkup.type === 'balita') {
            return `BB: ${checkup.weight}, TB/PB: ${checkup.height}, LILA: ${checkup.lila}, LK: ${checkup.lingkar_kepala}`;
        } else if (checkup.type === 'ibu-hamil') {
            return `BB: ${checkup.weight}, TD: ${checkup.bp}, LILA: ${checkup.lila}, UK: ${checkup.usia_kehamilan}, DJJ: ${checkup.djj}`;
        } else if (checkup.type === 'nifas') {
            return `BB: ${checkup.weight}, TD: ${checkup.bp}, LILA: ${checkup.lila}, Status Gizi: ${checkup.status_gizi}`;
        } else if (checkup.type === 'remaja') {
            return `BB: ${checkup.weight}, TB: ${checkup.height}, TD: ${checkup.bp}, LP: ${checkup.lingkar_perut}, Hb: ${checkup.hb}`;
        } else { // lansia
            return `BB: ${checkup.weight}, TD: ${checkup.bp}, LP: ${checkup.lingkar_perut}, Gula Darah: ${checkup.glucose}, Kolesterol: ${checkup.cholesterol}`;
        }
    }

    function getCheckupGridHtml(checkup) {
        if (checkup.type === 'balita') {
            return `
                <div><p class="text-gray-600 text-xs">Berat Badan</p><p class="font-semibold text-gray-800">${checkup.weight}</p></div>
                <div><p class="text-gray-600 text-xs">Tinggi/Panjang Badan</p><p class="font-semibold text-gray-800">${checkup.height}</p></div>
                <div><p class="text-gray-600 text-xs">LILA</p><p class="font-semibold text-gray-800">${checkup.lila}</p></div>
                <div><p class="text-gray-600 text-xs">Lingkar Kepala</p><p class="font-semibold text-gray-800">${checkup.lingkar_kepala}</p></div>
                <div><p class="text-gray-600 text-xs">Status Gizi</p><p class="font-semibold text-gray-800">${checkup.status_bb_pb}</p></div>
            `;
        } else if (checkup.type === 'ibu-hamil') {
            return `
                <div><p class="text-gray-600 text-xs">Berat Badan</p><p class="font-semibold text-gray-800">${checkup.weight}</p></div>
                <div><p class="text-gray-600 text-xs">Tinggi Badan</p><p class="font-semibold text-gray-800">${checkup.height}</p></div>
                <div><p class="text-gray-600 text-xs">Tekanan Darah</p><p class="font-semibold text-gray-800">${checkup.bp}</p></div>
                <div><p class="text-gray-600 text-xs">LILA</p><p class="font-semibold text-gray-800">${checkup.lila}</p></div>
                <div><p class="text-gray-600 text-xs">Usia Kehamilan</p><p class="font-semibold text-gray-800">${checkup.usia_kehamilan}</p></div>
            `;
        } else if (checkup.type === 'nifas') {
            return `
                <div><p class="text-gray-600 text-xs">Berat Badan</p><p class="font-semibold text-gray-800">${checkup.weight}</p></div>
                ...
                <div><p class="text-gray-600 text-xs">Tekanan Darah</p><p class="font-semibold text-gray-800">${checkup.bp}</p></div>
                <div><p class="text-gray-600 text-xs">LILA</p><p class="font-semibold text-gray-800">${checkup.lila}</p></div>
                <div><p class="text-gray-600 text-xs">Status Gizi</p><p class="font-semibold text-gray-800">${checkup.status_gizi}</p></div>
            `;
        } else if (checkup.type === 'remaja') {
            return `
                <div><p class="text-gray-600 text-xs">Berat Badan</p><p class="font-semibold text-gray-800">${checkup.weight}</p></div>
                <div><p class="text-gray-600 text-xs">Tinggi Badan</p><p class="font-semibold text-gray-800">${checkup.height}</p></div>
                <div><p class="text-gray-600 text-xs">Tekanan Darah</p><p class="font-semibold text-gray-800">${checkup.bp}</p></div>
                <div><p class="text-gray-600 text-xs">Lingkar Perut</p><p class="font-semibold text-gray-800">${checkup.lingkar_perut}</p></div>
                <div><p class="text-gray-600 text-xs">Hemoglobin</p><p class="font-semibold text-gray-800">${checkup.hb}</p></div>
            `;
        } else { // lansia
            return `
                <div><p class="text-gray-600 text-xs">Berat Badan</p><p class="font-semibold text-gray-800">${checkup.weight}</p></div>
                <div><p class="text-gray-600 text-xs">Tinggi Badan</p><p class="font-semibold text-gray-800">${checkup.height}</p></div>
                <div><p class="text-gray-600 text-xs">Tekanan Darah</p><p class="font-semibold text-gray-800">${checkup.bp}</p></div>
                <div><p class="text-gray-600 text-xs">Kolesterol</p><p class="font-semibold text-gray-800">${checkup.cholesterol}</p></div>
                <div><p class="text-gray-600 text-xs">Gula Darah</p><p class="font-semibold text-gray-800">${checkup.glucose}</p></div>
            `;
        }
    }
    ...
    function showMemberDetail(memberName) {
        ...
        // Render latest checkup fields dynamically
        const latestFieldsContainer = document.getElementById('latest-checkup-fields');
        let fieldsHtml = '';

        if (data.type === 'balita') {
            fieldsHtml = `
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Berat Badan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.weight}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tinggi/Panjang Badan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.height}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Lingkar Lengan (LILA)</p><p class="text-sm font-bold text-slate-800 mt-1">${data.lila}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Lingkar Kepala</p><p class="text-sm font-bold text-slate-800 mt-1">${data.lingkar_kepala}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Status Gizi</p><p class="text-sm font-bold text-slate-800 mt-1">${data.gizi}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tanggal Pemeriksaan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.checkupDate}</p></div>
            `;
        } else if (data.type === 'ibu-hamil') {
            fieldsHtml = `
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Berat Badan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.weight}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tinggi Badan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.height}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tekanan Darah</p><p class="text-sm font-bold text-slate-800 mt-1">${data.bp}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Lingkar Lengan (LILA)</p><p class="text-sm font-bold text-slate-800 mt-1">${data.lila}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Usia Kehamilan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.usia_kehamilan}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Detak Jantung Janin (DJJ)</p><p class="text-sm font-bold text-slate-800 mt-1">${data.djj}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tinggi Fundus</p><p class="text-sm font-bold text-slate-800 mt-1">${data.fundus}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tanggal Pemeriksaan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.checkupDate}</p></div>
            `;
        } else if (data.type === 'nifas') {
            fieldsHtml = `
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Berat Badan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.weight}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tinggi Badan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.height}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tekanan Darah</p><p class="text-sm font-bold text-slate-800 mt-1">${data.bp}</p></div>
                ...
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tanggal Pemeriksaan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.checkupDate}</p></div>
            `;
        } else if (data.type === 'remaja') {
            fieldsHtml = `
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Berat Badan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.weight}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tinggi Badan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.height}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tekanan Darah</p><p class="text-sm font-bold text-slate-800 mt-1">${data.bp}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Lingkar Perut</p><p class="text-sm font-bold text-slate-800 mt-1">${data.lingkar_perut}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Hemoglobin</p><p class="text-sm font-bold text-slate-800 mt-1">${data.hb}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Status IMT</p><p class="text-sm font-bold text-slate-800 mt-1">${data.gizi}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tanggal Pemeriksaan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.checkupDate}</p></div>
            `;
        } else { // lansia
            fieldsHtml = `
                ...
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tekanan Darah</p><p class="text-sm font-bold text-slate-800 mt-1">${data.bp}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Lingkar Perut</p><p class="text-sm font-bold text-slate-800 mt-1">${data.lingkar_perut}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Kolesterol</p><p class="text-sm font-bold text-slate-800 mt-1">${data.cholesterol}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Gula Darah</p><p class="text-sm font-bold text-slate-800 mt-1">${data.glucose}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Status Berat Badan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.gizi}</p></div>
                <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100"><p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Tanggal Pemeriksaan</p><p class="text-sm font-bold text-slate-800 mt-1">${data.checkupDate}</p></div>
            `;
        }
        latestFieldsContainer.innerHTML = fieldsHtml;
        ...
        checkupTable.innerHTML = memberCheckups.length ? memberCheckups.slice(0, 10).map(checkup => `<tr><td class="px-4 py-3 text-gray-700">${checkup.date}</td><td class="px-4 py-3 text-gray-700">Pemeriksaan Kesehatan</td><td class="px-4 py-3"><span class="${checkup.status === 'Sehat' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'} px-2 py-1 rounded text-xs">${checkup.status}</span></td><td class="px-4 py-3 text-gray-600">${getCheckupDetailText(checkup)}</td><td class="px-4 py-3"><a href="/kepala-keluarga/pemeriksaan/${checkup.type}/${checkup.id}/print" target="_blank" class="inline-flex items-center px-2 py-1 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 rounded text-xs font-semibold transition"><i class="fa-solid fa-print mr-1"></i> Cetak</a></td></tr>`).join('') : '<tr><td colspan="5" class="px-4 py-3 text-gray-500 text-center">Belum ada data pemeriksaan.</td></tr>';
        if (checkupMobile && mobileTemplate) {
            checkupMobile.innerHTML = memberCheckups.length ? memberCheckups.slice(0, 10).map(checkup => {
                ...
                node.querySelector('.member-checkup-note-mobile').textContent = getCheckupDetailText(checkup);
                node.querySelector('.member-checkup-print-mobile').href = `/kepala-keluarga/pemeriksaan/${checkup.type}/${checkup.id}/print`;
