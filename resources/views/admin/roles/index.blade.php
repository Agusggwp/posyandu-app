...
<div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex justify-between items-center mb-8">
            ...
            <a href="{{ route('roles.create') }}" class="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-all shadow-sm">
                    ...
                    <div class="flex gap-2 pt-4 border-t border-gray-100">
                        ...
                        @if(!in_array($role->name, ['Admin', 'Kader', 'Bidan']))
                            <form action="{{ route('roles.destroy', $role) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this role?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="px-4 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors">
                                    Delete
                                </button>
                            </form>
                        @else
                            <button type="button" class="px-4 py-2 bg-gray-100 text-gray-400 rounded-lg cursor-not-allowed" disabled title="Cannot delete default role">
                                Delete
                            </button>
                        @endif
                    </div>
